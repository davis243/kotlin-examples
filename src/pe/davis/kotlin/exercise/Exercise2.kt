package pe.davis.kotlin.exercise
...
data class Seat(val row: Int, val num: Int, val price: BigDecimal, val description: String){
 ...
 class Theater {
     private val hiddenSeat = mutableListOf<Seat>()
     val seats
     get() = hiddenSeat.toList()
     ...
     constructor() {
         ...
         for( row in 1..15) {
             for(num in 1..36)
                 hiddenSeat.add(Seat(row, num,getPrice(row, num), getDescription(row, num) ))
         }
     }
 }
